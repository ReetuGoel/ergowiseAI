...
export function LoginForm({ onSwitch }: { onSwitch: () => void }) {
  ...
  return (
    <form onSubmit={submit} style={formStyle}>
      <h2 style={titleStyle}>Log in to ErgoWise</h2>
      ...
      <button style={buttonStyle} disabled={loading}>{loading ? 'Logging in...' : 'Log In'}</button>
      <p style={{ textAlign: 'center', marginTop: 12, fontSize: 14 }}>No account? <span style={linkStyle} onClick={onSwitch}>Sign up</span></p>
    </form>
  );
}
